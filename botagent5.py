...
class SpiderConfig:
    ...
    # Head configuration
    head_pivot_name: str = "Head_Pivot"
    head_default_name: str = "Head_Default"
...
class SpiderRigBuilder:
    ...
    def build(self, clear_scene: bool = True) -> bool:
        ...
        print("=" * 60)
        print("SPIDER RIG BUILDER (ALL-IN-ONE) - Starting Build (BotAgent5)")
        print("=" * 60)
        ...
        try:
            ...
            steps = [
                ...
                ("Creating Head Assembly", self._create_head_assembly),
                ...
                ("Setting Up Head Swapping", self._setup_head_swapping),
    ...
    def _create_master_controller(self):
        ...
        look_target = bpy.data.objects.get(self.config.look_target_name)
        if look_target:
            children_to_parent.append(look_target)
    ...
    def _create_head_assembly(self):
        """Create the surveillance head/turret."""
        BlenderContext.deselect_all()
        
        body = bpy.data.objects.get(self.config.body_name)
        if not body:
            print("    Warning: No body found for head attachment")
            return

        # 1. Create Pivot (The 'Neck')
        bpy.ops.object.empty_add(type='CUBE', radius=0.5, location=(0, 0, 0))
        pivot = bpy.context.active_object
        pivot.name = self.config.head_pivot_name
        pivot.empty_display_size = 0.5
        pivot[self.config.component_key] = self.config.head_tag
        
        # Place on top of body
        pivot.location = body.location.copy()
        pivot.location.z += 1.2 
        
        # Parent to Body
        pivot.parent = body
        pivot.matrix_parent_inverse = body.matrix_world.inverted()
        
        # 2. Create Look Target
        target_loc = pivot.location.copy()
        target_loc.y -= 5.0 # Look forward
        target_loc.z -= 1.0 
        
        bpy.ops.object.empty_add(type='SPHERE', radius=0.5, location=target_loc)
        look_target = bpy.context.active_object
        look_target.name = self.config.look_target_name
        look_target.empty_display_size = 0.5
        look_target[self.config.component_key] = self.config.controller_tag
        
        # 3. Add Track Constraint to Pivot
        c = pivot.constraints.new('TRACK_TO')
        c.target = look_target
        c.track_axis = 'TRACK_NEGATIVE_Y'
        c.up_axis = 'UP_Z'
        
        # 4. Create Generic Head Geometry
        bpy.ops.mesh.primitive_cube_add(size=0.6, location=(0,0,0))
        head_geo = bpy.context.active_object
        head_geo.name = self.config.head_default_name
        
        # Parent to Pivot
        head_geo.location = pivot.location
        head_geo.parent = pivot
        head_geo.matrix_parent_inverse = pivot.matrix_world.inverted()
        
        # Shape it like a scanner head
        head_geo.scale = Vector((0.8, 1.2, 0.5))
        head_geo.rotation_euler.x = math.radians(10) # Tilt slightly
        
        head_geo[self.config.component_key] = self.config.head_tag

        print(f"    Created Head Assembly: {pivot.name}, {look_target.name}")

    def _setup_head_swapping(self):
        """Setup logic to swap the generic head with a user custom object."""
        master = bpy.data.objects.get(self.config.master_controller_name)
        if not master: return
        
        # Properties
        master["use_custom_head"] = 0
        master["custom_head_name"] = "" 
        
        # UI
        try:
            ui = master.id_properties_ui("use_custom_head")
            ui.update(min=0, max=1, description="0=Generic, 1=Custom Object", default=0)
        except: pass
        
        self._register_head_swap_handler(
            master.name,
            self.config.head_pivot_name,
            self.config.head_default_name
        )
        print("    Added 'use_custom_head' and 'custom_head_name' properties")
        
    def _register_head_swap_handler(self, master_name, pivot_name, default_head_name):
        handler_name = "spider_head_swap_handler"
        for handler in bpy.app.handlers.depsgraph_update_post[:]:
            if hasattr(handler, '__name__') and handler.__name__ == handler_name:
                bpy.app.handlers.depsgraph_update_post.remove(handler)
        
        def spider_head_swap_handler(scene, depsgraph):
            master = bpy.data.objects.get(master_name)
            pivot = bpy.data.objects.get(pivot_name)
            default_head = bpy.data.objects.get(default_head_name)
            
            if not all([master, pivot, default_head]): return
            
            use_custom = master.get("use_custom_head", 0) > 0.5
            custom_name = master.get("custom_head_name", "")
            
            # Default visibility
            default_head.hide_viewport = use_custom
            default_head.hide_render = use_custom
            
            if custom_name:
                custom_obj = bpy.data.objects.get(custom_name)
                if custom_obj:
                    c_name = "Spider_Head_Attach"
                    c = custom_obj.constraints.get(c_name)
                    
                    if use_custom:
                        if not c:
                            c = custom_obj.constraints.new('COPY_TRANSFORMS')
                            c.name = c_name
                            c.target = pivot
                        c.mute = False
                    else:
        ...
        spider_head_swap_handler.__name__ = handler_name
        bpy.app.handlers.depsgraph_update_post.append(spider_head_swap_handler)    
