"""
Spider Rig Builder - All-in-One Version (botagent3)
===================================================
    ...
    def _register_random_walk_handler(self, master_name: str):
        ...
        def spider_random_walk_handler(scene):
            ...
            if master.get("random_walk", 0) < 0.5:
                # If disabled, ensure we stop any lingering velocity? 
            ...
            # noise.noise returns float ~ [-1, 1] or [0, 1] depending on implementation
            # mathutils.noise.noise(position)
            ...
            # Assuming 'Forward' is Local X-axis? Let's assume Local X (Red Arrow).
            # If so, movement is X = cos(rot_z), Y = sin(rot_z)
            
            # Current global rotation Z
